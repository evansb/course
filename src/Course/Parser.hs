...
mapParser ::
  ...
  -> Parser a
  -> Parser b
mapParser f p0 = P (\input -> case parse p0 input of
                        ErrorResult someError -> ErrorResult someError
                        Result consumed res -> Result consumed (f res))
...
bindParser ::
  ...
  -> Parser a
  -> Parser b
bindParser f p0 = P (\input -> case parse p0 input of
                    ErrorResult someError -> ErrorResult someError
                    Result consumed value -> parse (f value) consumed)
...
(|||) ::
  ...
  -> Parser a
  -> Parser a
p0 ||| p1 = P (\input -> case parse p0 input of
                ErrorResult _ -> parse p1 input
                Result consumed value -> Result consumed value)
...
natural = do
            value <- list1 digit
            case read value :: Optional Int of
                   Full v -> return v
                   Empty -> failed
...
sequenceParser (p :. prest) = do v <- p
                                 vv <- sequenceParser prest
                                 return (v:. vv)
